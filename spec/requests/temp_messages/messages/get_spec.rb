...
RSpec.describe 'Messages API', type: :request do
  describe 'GET /api/temp-messages/messages/:id' do
    ...
    context 'when the record exists' do
      ...
      it 'returns ok status' do
        get "/api/temp-messages/messages/#{sample_message.id}"
    ...
    context 'when the record does not exist' do
      ...
      it 'returns status code 404' do
        get "/api/temp-messages/messages/#{message_id}"
    ...
    context 'when the record is expired' do
      ...
      it 'returns status code 401' do
        get "/api/temp-messages/messages/#{expired_message.id}"
    ...
    context 'when the record has an invalid expiration type' do
      ...
      it 'returns status code 422' do
        get "/api/temp-messages/messages/#{invalid_message.id}"
